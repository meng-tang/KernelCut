...
% samplerate=0 corresponds to the case without group prior
samplerate = 1
...
while sum(swap(:))~=0
    ...
    for alpha = randperm(opt.numClusters)
        for beta = randperm(opt.numClusters)
            ...
            roi = [s_alpha ; s_beta]';
            center_alpha = sum( embedding(s_alpha,:),1 ) / numel(s_alpha);
            center_beta = sum( embedding(s_beta,:),1 ) / numel(s_beta);
            dist_to_alpha = sum( (embedding - repmat(center_alpha,N,1) ).^2 , 2);
            dist_to_beta = sum( (embedding - repmat(center_beta,N,1) ).^2 , 2);
            ...
            if diffcount > 2 %&& ( current_e - new_e) > 1e-6
                ...
                new_e = scutenergy(embedding, Labeling, opt, G, goodgroups);
                current_e = new_e;
                %disp(['diffcount ' num2str(diffcount)]);
                scutES = [scutES current_e];
...
save('scut.mat','Labeling','nmis');
disp(['nmi of spectral cut: ' num2str(nmi(Labeling,labels))]);
disp(['ncut energy: ' num2str(ncutEnergy( W, Labeling(:) ))]);
return
figure;plot(nmis);xlabel('Iteration');ylabel('NMI');
set(gca,'FontSize',16);
figure;plot(scutES);xlabel('Iteration');ylabel('K-means + Group Prior Energy');
set(gca,'FontSize',16);
